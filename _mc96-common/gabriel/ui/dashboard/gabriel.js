...
const GABRIEL = {
  ...
  heavenUrl: 'https://heaven.noizy.ai',
  heavenFallback: 'https://heaven.rsp-5f3.workers.dev',
...
const HEAVEN_LAUNCH = new Date('2026-04-17T00:00:00Z');
...
function initGabriel() {
  ...
  checkGabrielStatus();
  checkHeavenHealth();
  // Pulse the status every 15s (Heaven on same cadence; CF edge cache is 30s so we're gentle)
  setInterval(checkGabrielStatus, 15000);
  setInterval(checkHeavenHealth, 15000);
...
// ── HEAVEN HEALTH CHECK (NEXT_25 Move 17) ──────────────────────
// Polls /api/health — no auth required, CORS public, returns:
//   { status, service, version, env, timestamp, gabriel, hvs, portals, features:{...} }
// Falls back to workers.dev URL if Custom Domain hasn't propagated yet.

const esc = (s) => String(s == null ? '' : s).replace(/[&<>"']/g,
  (c) => ({ '&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;' }[c]));

async function checkHeavenHealth() {
  const pill = document.getElementById('heavenPill');
  const panel = document.getElementById('heavenPanel');
  if (pill) pill.innerHTML = '<div class="dot warn"></div> HEAVEN …';

  let data = null;
  let sourceUrl = GABRIEL.heavenUrl;
  const fetchHeaven = async (base) => {
    const r = await fetch(`${base}/api/health`, { signal: AbortSignal.timeout(4000), cache: 'no-store' });
    if (!r.ok) throw new Error(`HTTP ${r.status}`);
    return r.json();
  };

  try {
    data = await fetchHeaven(GABRIEL.heavenUrl);
  } catch (_) {
    try {
      data = await fetchHeaven(GABRIEL.heavenFallback);
      sourceUrl = GABRIEL.heavenFallback;
    } catch (_) { /* both down */ }
  }

  GABRIEL.heaven = data;

  // Pill update — single source of status truth
  if (pill) {
    if (data) {
      const maint = data.features && data.features.maintenance === true;
      pill.innerHTML = `<div class="dot ${maint ? 'warn' : 'live'}"></div> HEAVEN v${esc(data.version)}`;
      pill.title = `${sourceUrl} • ${data.env} • ${new Date(data.timestamp).toLocaleTimeString()}`;
    } else {
      pill.innerHTML = '<div class="dot off"></div> HEAVEN DOWN';
      pill.title = 'Both heaven.noizy.ai and heaven.rsp-5f3.workers.dev unreachable';
    }
  }

  // Panel render — TODO: user decides what to surface here
  if (panel) panel.innerHTML = renderHeavenPanel(data, sourceUrl);

  // Log into Gabriel event stream
  addGabrielLog(
    data ? `HEAVEN ✓ v${data.version} (${data.env})` : 'HEAVEN ✗ health check failed',
    data ? '' : 'warn'
  );
}

// TODO (learning-mode): shape the live Heaven panel.
// The fetcher above already gives you a full payload. Decide what creators care about.
// Return an HTML string. Keep it tight — this block sits above the voice bar.
// Use esc() on any dynamic value you interpolate.
function renderHeavenPanel(data, sourceUrl) {
  if (!data) {
    return `<span style="color:var(--red)">⬤</span> Heaven unreachable — last known status unavailable`;
  }
  ...
  //   data.version     "18.1.0"
  //   data.env         "production"
  //   data.gabriel     "watching"
  //   data.hvs         "75/25 perpetual"
  //   data.portals     ["NOIZYVOX","NOIZYFISH","NOIZYKIDZ","NOIZYLAB","WISDOM","myFAMILY"]
  //   data.features    { ai_inference, voice_vault, gap_solver, maintenance }
  //   sourceUrl        which Heaven origin answered
  //   HEAVEN_LAUNCH    Date — April 17 countdown target
  //
  // Replace the placeholder below with the rendering you want.
  return `
    <div style="display:flex;gap:14px;align-items:center;flex-wrap:wrap">
      <span style="color:var(--green)">⬤</span>
      <strong>v${esc(data.version)}</strong>
      <span style="color:var(--muted)">· ${esc(data.env)}</span>
      <span style="color:var(--subtle)">· ${esc(sourceUrl.replace('https://',''))}</span>
    </div>`;
}
